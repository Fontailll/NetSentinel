...
bool WindowsFirewall::block_ip(const std::string& ip) {
    ...
    std::string name = "NetSentinel Block " + ip;
    _bstr_t bstrName(name.c_str());
    _bstr_t bstrDesc = L"Blocked by NetSentinel Network Incident Response Tool";
...
bool WindowsFirewall::unblock_ip(const std::string& ip) {
    ...
    _bstr_t bstrOutName(("NetSentinel Block " + ip).c_str());
    _bstr_t bstrInName(("NetSentinel Block In " + ip).c_str());
...
bool WindowsFirewall::block_port(uint16_t port) {
    ...
    std::string name = "NetSentinel Block Port " + std::to_string(port);
    std::string port_str = std::to_string(port);
    _bstr_t bstrName(name.c_str());
    _bstr_t bstrDesc = L"Blocked by NetSentinel";
